...
check('net producer revenue is visible', page.includes('Líquido do produtor') && page.includes('summary.producer_net_revenue'))
check('availability metric is visible', page.includes('Disponíveis') && page.includes('summary.available_count'))
check('occupancy and average ticket metrics are visible', page.includes('Ocupação') && page.includes('summary.occupancy_rate') && page.includes('Ticket médio') && page.includes('summary.average_ticket_value'))
...
check('commercial timeline is rendered', page.includes('Vendas nos últimos dias') && page.includes('details?.sales_timeline'))
check('checkout funnel is rendered', page.includes('Funil a partir do checkout') && page.includes('payment_conversion_rate'))
check('financial breakdown is rendered', page.includes('Taxa da plataforma') && page.includes('Taxa do processador') && page.includes('financial?.producer_net'))
check('payment methods are rendered', page.includes('Meios utilizados') && page.includes('details.payment_methods'))
check('operational alerts exist', page.includes('Evento perto de esgotar') && page.includes('Conversão de pagamento baixa'))
check('auto refresh is enabled with operational cadence', page.includes('LIVE_REFRESH_MS = 15000') && page.includes('setInterval') && page.includes('Atualização automática ativa'))
check('event filters exist', page.includes('Buscar evento ou local') && page.includes('Todos os status'))
check('sales operational filters exist', page.includes('Titular, e-mail, pedido ou ingresso') && page.includes('Todos os pagamentos') && page.includes('Todos os check-ins'))
check('csv export exists', page.includes('downloadCsv') && page.includes('Exportar CSV') && page.includes('text/csv'))
...
check('advanced panels remain responsive', css.includes('.ats-dashboard-grid') && css.includes('.ats-financial-grid') && css.includes('.ats-sale-filters'))
