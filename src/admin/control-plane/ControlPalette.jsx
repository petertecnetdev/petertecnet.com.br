...
export default function ControlPalette({ open, onClose, onOpenEntity, history = [], favorites = [], onHistory, onFavorites }) {
  ...
  const entries = useMemo(() => [
    ...navigation.map(item => ({ kind: 'nav', item })),
    ...searchEntries,
  ], [navigation, searchEntries])
  ...
  function remember(entry) {
    const fingerprint = entry.kind === 'nav'
      ? `nav:${entry.item[0]}`
      : `entity:${entry.group}:${entry.row?.id}`
    const label = entry.kind === 'nav'
      ? entry.item[1]
      : entry.row?.name || entry.row?.title || entry.row?.email || `${entry.group} #${entry.row?.id}`
    const next = [{ fingerprint, label, entry }, ...history.filter(item => item.fingerprint !== fingerprint)].slice(0, 8)
    onHistory?.(next)
  }
  ...
  function onKeyDown(event) {
    if (event.key === 'ArrowDown') { event.preventDefault(); setSelected(value => Math.min(entries.length - 1, value + 1)) }
  ...
  return <div className="cp-overlay" role="dialog" aria-modal="true" aria-label="Central de comandos" onMouseDown={event => event.target === event.currentTarget && onClose()}>
    <section className="cp-palette">
      <header>
        <span>⌕</span>
        <input ref={inputRef} value={query} onChange={event => setQuery(event.target.value)} onKeyDown={onKeyDown} placeholder="Navegar, localizar entidade ou executar comando…" />
        <kbd>ESC</kbd>
      </header>

      <div className="cp-palette-body">
        {!query && !!favorites.length && <section><h4>Favoritos</h4>{favorites.slice(0, 6).map(item => <button type="button" key={item.fingerprint} onClick={() => activate(item.entry)}><span className="cp-entry-icon">★</span><span><b>{item.label}</b><small>Acesso favorito</small></span><em>↵</em></button>)}</section>}
        {!query && !!history.length && <section><h4>Recentes</h4>{history.slice(0, 5).map(item => <button type="button" key={item.fingerprint} onClick={() => activate(item.entry)}><span className="cp-entry-icon">↺</span><span><b>{item.label}</b><small>Aberto recentemente</small></span><em>↵</em></button>)}</section>}

        <section><h4>Navegação</h4>{navigation.map((item, index) => {
          const entry = { kind: 'nav', item }
          const fingerprint = `nav:${item[0]}`
          return <button type="button" className={entries[selected] === entries[index] ? 'selected' : ''} key={item[0]} onMouseEnter={() => setSelected(index)} onClick={() => activate(entry)}>
            <span className="cp-entry-icon">{favoriteFingerprints.has(fingerprint) ? '★' : '◇'}</span><span><b>{item[1]}</b><small>{item[2]}</small></span><button type="button" className="cp-star" onClick={event => toggleFavorite(entry, event)} aria-label="Alternar favorito">☆</button><em>↵</em>
          </button>
        })}</section>
        ...
        {query.trim().length >= 2 && <section><h4>Busca global {loading && <small>consultando…</small>}</h4>{!loading && !searchEntries.length && <p className="cp-empty">Nenhuma entidade encontrada.</p>}{searchEntries.map((entry, resultIndex) => {
          ...
          return <button type="button" className={selected === index ? 'selected' : ''} key={fingerprint} onMouseEnter={() => setSelected(index)} onClick={() => activate(entry)}>
            <span className="cp-entry-icon">{favoriteFingerprints.has(fingerprint) ? '★' : '⌁'}</span><span><b>{title}</b><small>{entry.group} · {meta}</small></span><button type="button" className="cp-star" onClick={event => toggleFavorite(entry, event)} aria-label="Alternar favorito">☆</button><em>→</em>
          </button>
        })}</section>}
      </div>
