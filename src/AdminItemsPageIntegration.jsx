...
export default function AdminItemsPageIntegration() {
  ...
  useEffect(() => {
    ...
    const mount = () => {
      ...
      if (!navButton) {
        ...
        const before = Array.from(sidebarNav.querySelectorAll('button')).find(button => button.textContent?.includes('Notificações') || button.textContent?.includes('Atividade'))
  ...
  return host ? createPortal(<AdminItemsPage />, host) : null
}
