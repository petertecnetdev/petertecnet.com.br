...
const GENERIC_LABELS = new Set(["imagem","image","foto","photo","avatar","logo","icone","ícone","item","produto","product","servico","serviço","service","estabelecimento","empresa","company"]);

function normalizeFallbackLabel(value) {
  const raw = String(value || "").replace(/\s+/g, " ").trim();
  if (!raw) return "";
  const cleaned = raw.replace(/^(?:imagem|image|foto|photo|avatar|logo|icone|ícone)(?:\s+(?:de|do|da|dos|das))?\s*/i, "").replace(/\s+/g, " ").trim();
...
export function getImageFallbackInitials(value) {
  ...
  if (!label) return "?";
  const ascii = label.normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-zA-Z0-9\s'-]/g, " ").replace(/\s+/g, " ").trim();
  if (!ascii) return "?";
...
function getCandidateLabel(element) {
  if (!element) return "";
  const candidates = [element.dataset && element.dataset.fallbackText, element.dataset && element.dataset.name, element.dataset && element.dataset.title, element.getAttribute && element.getAttribute("alt"), element.getAttribute && element.getAttribute("aria-label"), element.getAttribute && element.getAttribute("title")];
  for (const candidate of candidates) { const normalized = normalizeFallbackLabel(candidate); if (normalized) return normalized; }
  return "";
}
...
function applyInitialsFallback(image, failedSrc) {
  ...
  if (image.dataset && !image.dataset.imageFallbackOriginalSrc && failedSrc && !isFallbackDataUri(failedSrc)) image.dataset.imageFallbackOriginalSrc = failedSrc;
...
export function installGlobalImageFallbacks() {
  ...
  const observer = new MutationObserver((mutations) => mutations.forEach((mutation) => mutation.type === "attributes" ? inspectImage(mutation.target) : mutation.addedNodes.forEach(inspectNode)));
  ...
  const cleanup = () => { document.removeEventListener("error", handleError, true); observer.disconnect(); delete window[INSTALL_FLAG]; };
