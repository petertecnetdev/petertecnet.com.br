...
function IdentityImage({ src, label, kind = 'user' }) {
  const [failed, setFailed] = useState(false)
  const initials = String(label || '?').split(/\s+/).filter(Boolean).slice(0, 2).map(part => part[0]).join('').toUpperCase()
  return <span className={`identity-image ${kind}`}>{src && !failed ? <img src={src} alt={label || ''} onError={() => setFailed(true)} /> : <b>{initials || '?'}</b>}</span>
}
...
function Activity({ payload, applications, users, load, setData, activityBase = '/admin/ecosystem/activity' }) {
  const [filters, setFilters] = useState({ user_id: '', app_id: '', type: '', outcome: '', severity: '', environment: '', method: '', entity_type: '', from: '', to: '', search: '' })
  ...
  return <div className="admin-stack"><div className="metric-grid three"><Metric label="Interações" value={payload?.summary?.total} sub="em todo o histórico filtrado" /><Metric label="Usuários" value={payload?.summary?.users} sub="usuários únicos" /><Metric label="Aplicações" value={payload?.summary?.applications} sub="com atividade" /></div><Panel title="Filtros de atividade"><form className="filter-grid" onSubmit={apply}><Field label="Busca" value={filters.search} onChange={value => setFilters({ ...filters, search: value })} /><label>Aplicação<select value={filters.app_id} onChange={event => setFilters({ ...filters, app_id: event.target.value })}><option value="">Todas</option>{applications?.applications?.map(application => <option key={application.id} value={application.id}>{application.name}</option>)}</select></label><label>Usuário<select value={filters.user_id} onChange={event => setFilters({ ...filters, user_id: event.target.value })}><option value="">Todos</option>{users?.users?.map(user => <option key={user.id} value={user.id}>{fullName(user)} · {user.email}</option>)}</select></label><label>Ação<select value={filters.type} onChange={event => setFilters({ ...filters, type: event.target.value })}><option value="">Todas</option>{payload?.types?.map(type => <option key={type} value={type}>{humanType(type)}</option>)}</select></label><label>Resultado<select value={filters.outcome} onChange={event => setFilters({ ...filters, outcome: event.target.value })}><option value="">Todos</option><option value="success">Sucesso</option><option value="denied">Recusado</option><option value="error">Erro</option></select></label><label>Severidade<select value={filters.severity} onChange={event => setFilters({ ...filters, severity: event.target.value })}><option value="">Todas</option><option value="normal">Normal</option><option value="attention">Atenção</option><option value="suspicious">Suspeita</option><option value="critical">Crítica</option></select></label><label>Ambiente<select value={filters.environment} onChange={event => setFilters({ ...filters, environment: event.target.value })}><option value="">Todos</option><option value="production">Produção</option><option value="staging">Staging</option><option value="development">Desenvolvimento</option></select></label><label>Método HTTP<select value={filters.method} onChange={event => setFilters({ ...filters, method: event.target.value })}><option value="">Todos</option>{['GET','POST','PUT','PATCH','DELETE'].map(method => <option key={method}>{method}</option>)}</select></label><Field label="Tipo da entidade" value={filters.entity_type} onChange={value => setFilters({ ...filters, entity_type: value })} /><Field label="De" type="date" value={filters.from} onChange={value => setFilters({ ...filters, from: value })} /><Field label="Até" type="date" value={filters.to} onChange={value => setFilters({ ...filters, to: value })} /><div className="form-actions"><button className="primary">Aplicar filtros</button><button type="button" onClick={() => setFilters({ user_id: '', app_id: '', type: '', outcome: '', severity: '', environment: '', method: '', entity_type: '', from: '', to: '', search: '' })}>Limpar</button></div></form></Panel><Panel title="Todas as interações"><Timeline rows={payload?.activity || []} /><div className="infinite-scroll-status" ref={loadMoreRef}>{loadingMore ? <><i />Carregando mais interações...</> : payload?.pagination?.has_more ? 'Continue rolando para carregar mais' : `Fim do histórico · ${payload?.activity?.length || 0} de ${payload?.pagination?.total || 0}`}</div></Panel></div>
}
...
function AdministratorUsers({ payload, profiles, applications, load, mutate, openUser }) {
  const [form, setForm] = useState(emptyUser); const [editing, setEditing] = useState(null); const [filters, setFilters] = useState({ search: '', profile_id: '', app_id: '', access_status: '', has_establishment: '', activity_from: '', activity_to: '' })
  const users = payload?.users || []
  useEffect(() => { if (!profiles) load('profiles'); if (!applications) load('applications') }, [])
  async function doSearch(e) { e.preventDefault(); const query = new URLSearchParams(Object.entries(filters).filter(([, value]) => value)); await load('users', `/admin/ecosystem/users?${query}`) }
  ...
  return <div className="admin-stack"><div className="admin-grid"><Panel title={editing ? 'Editar usuário' : 'Novo usuário'}><form onSubmit={submit} className="form-grid"><Field label="Nome" value={form.first_name} onChange={v => setForm({ ...form, first_name: v })} required /><Field label="Sobrenome" value={form.last_name} onChange={v => setForm({ ...form, last_name: v })} /><Field label="Usuário" value={form.user_name} onChange={v => setForm({ ...form, user_name: v })} required /><Field label="E-mail" value={form.email} onChange={v => setForm({ ...form, email: v })} required /><label className="wide">Perfil<select value={form.profile_id} onChange={e => setForm({ ...form, profile_id: e.target.value })}><option value="">Sem perfil</option>{profiles?.profiles?.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select></label>{!editing && <Field label="Senha inicial" type="password" value={form.password} onChange={v => setForm({ ...form, password: v })} wide required />}<div className="form-actions wide"><button className="primary">Salvar usuário</button>{editing && <button type="button" onClick={() => { setEditing(null); setForm(emptyUser) }}>Cancelar</button>}</div></form></Panel><Panel title="Filtrar usuários"><form className="form-grid" onSubmit={doSearch}><Field label="Nome, e-mail, usuário ou ID" value={filters.search} onChange={value => setFilters({ ...filters, search: value })} wide /><label>Perfil<select value={filters.profile_id} onChange={event => setFilters({ ...filters, profile_id: event.target.value })}><option value="">Todos</option>{profiles?.profiles?.map(profile => <option key={profile.id} value={profile.id}>{profile.name}</option>)}</select></label><label>Aplicação<select value={filters.app_id} onChange={event => setFilters({ ...filters, app_id: event.target.value })}><option value="">Todas</option>{applications?.applications?.map(app => <option key={app.id} value={app.id}>{app.name}</option>)}</select></label><label>Status do acesso<select value={filters.access_status} onChange={event => setFilters({ ...filters, access_status: event.target.value })}><option value="">Todos</option><option value="active">Ativo</option><option value="blocked">Bloqueado</option><option value="pending">Pendente</option><option value="none">Sem vínculo na aplicação</option></select></label><label>Possui empresa<select value={filters.has_establishment} onChange={event => setFilters({ ...filters, has_establishment: event.target.value })}><option value="">Todos</option><option value="yes">Sim</option><option value="no">Não</option></select></label><Field label="Atividade desde" type="date" value={filters.activity_from} onChange={value => setFilters({ ...filters, activity_from: value })} /><Field label="Atividade até" type="date" value={filters.activity_to} onChange={value => setFilters({ ...filters, activity_to: value })} /><div className="form-actions wide"><button className="primary">Aplicar filtros</button><button type="button" onClick={() => setFilters({ search: '', profile_id: '', app_id: '', access_status: '', has_establishment: '', activity_from: '', activity_to: '' })}>Limpar</button></div></form><p className="helper">Combine os critérios para localizar públicos e acessos específicos.</p></Panel></div><Panel title={`Usuários (${users.length})`}><div className="user-list">{users.map(user => <article className="user-card" key={user.id}><div className="user-head"><div className="user-identity"><IdentityImage src={user.avatar} label={fullName(user)} /><div><b>{fullName(user)}</b><span>{user.email} · {user.profile?.name || 'Sem perfil'}</span><small>Última atividade: {fmt(user.last_activity_at)} · {user.interactions_count || 0} interações · {user.establishments_count || 0} estabelecimentos</small></div></div><div><button className="primary" onClick={() => openUser(user)}>Detalhes</button><button onClick={() => edit(user)}>Editar</button><button className="danger" onClick={() => window.confirm(`Excluir ${user.email}?`) && mutate(`/admin/ecosystem/users/${user.id}`, { method: 'DELETE' }, 'Usuário excluído.', 'users')}>Excluir</button></div></div><div className="access-grid">{applications?.applications?.map(app => { const link = user.applications?.find(a => a.id === app.id); const active = link?.pivot?.status === 'active'; return <button key={app.id} className={active ? 'access active' : 'access'} onClick={() => access(user, app)}><b>{app.name}</b><small>{link ? (active ? 'Permitido' : link.pivot.status) : 'Sem vínculo'}</small></button> })}</div></article>)}</div></Panel></div>
}
...
function Applications({ payload, mutate }) {
  const [form, setForm] = useState(emptyApp); const [editing, setEditing] = useState(null); const [filters, setFilters] = useState({ search: '', status: '' }); const apps = payload?.applications || []
  const visibleApps = apps.filter(app => {
    const search = filters.search.trim().toLowerCase()
    const matchesSearch = !search || [app.name, app.slug, app.url, app.author].some(value => String(value || '').toLowerCase().includes(search))
    const matchesStatus = !filters.status || (filters.status === 'active' ? app.is_active : !app.is_active)
...
function Establishments({ payload, applications, users, load, mutate }) {
  ...
  async function doSearch(event) {
    ...
    await load('establishments', `/admin/ecosystem/establishments?${query}`)
  }
  ...
  return <div className="admin-stack">
    <div className="admin-grid">
      ...
      <Panel title="Pesquisar empresas">
        <form className="form-grid" onSubmit={doSearch}><Field label="Nome, CNPJ, e-mail, telefone ou ID" value={filters.search} onChange={value => setFilters({ ...filters, search: value })} wide /><label>Aplicação<select value={filters.app_id} onChange={event => setFilters({ ...filters, app_id: event.target.value })}><option value="">Todas</option>{appRows.map(app => <option key={app.id} value={app.id}>{app.name}</option>)}</select></label><label>Responsável<select value={filters.user_id} onChange={event => setFilters({ ...filters, user_id: event.target.value })}><option value="">Todos</option>{userRows.map(user => <option key={user.id} value={user.id}>{fullName(user)}</option>)}</select></label><label>Aprovação<select value={filters.approval} onChange={event => setFilters({ ...filters, approval: event.target.value })}><option value="">Todas</option><option value="approved">Aprovada</option><option value="pending">Pendente</option></select></label><label>Publicação<select value={filters.publication} onChange={event => setFilters({ ...filters, publication: event.target.value })}><option value="">Todas</option><option value="published">Publicada</option><option value="hidden">Oculta</option></select></label><label>Destaque<select value={filters.featured} onChange={event => setFilters({ ...filters, featured: event.target.value })}><option value="">Todos</option><option value="yes">Em destaque</option><option value="no">Sem destaque</option></select></label><Field label="Cidade" value={filters.city} onChange={value => setFilters({ ...filters, city: value })} /><Field label="UF" value={filters.uf} onChange={value => setFilters({ ...filters, uf: value.slice(0, 2) })} /><Field label="Tipo" value={filters.type} onChange={value => setFilters({ ...filters, type: value })} /><Field label="Categoria" value={filters.category} onChange={value => setFilters({ ...filters, category: value })} /><div className="form-actions wide"><button className="primary">Aplicar filtros</button><button type="button" onClick={() => setFilters({ search: '', app_id: '', user_id: '', approval: '', publication: '', featured: '', city: '', uf: '', type: '', category: '' })}>Limpar</button></div></form>
        <p className="helper">Combine aplicação, responsável, localização e situação de publicação.</p>
      </Panel>
    </div>
...
function Items({ payload, establishments, applications, load, mutate }) {
  ...
  const [editing, setEditing] = useState(null)
  const [filters, setFilters] = useState({ search: '', app_id: '', establishment_id: '', type: '', status: '', featured: '', category: '', min_price: '', max_price: '' })
    ...
    <div className="admin-grid">
      ...
      <Panel title="Filtrar itens">
        <form className="form-grid" onSubmit={apply}>
          ...
          <label>Tipo<select value={filters.type} onChange={event => setFilters({ ...filters, type: event.target.value })}><option value="">Todos</option><option value="product">Produto</option><option value="service">Serviço</option><option value="item">Item</option><option value="ticket">Ingresso</option></select></label><label>Status<select value={filters.status} onChange={event => setFilters({ ...filters, status: event.target.value })}><option value="">Todos</option><option value="active">Ativo</option><option value="archived">Arquivado</option></select></label><label>Destaque<select value={filters.featured} onChange={event => setFilters({ ...filters, featured: event.target.value })}><option value="">Todos</option><option value="yes">Em destaque</option><option value="no">Sem destaque</option></select></label><Field label="Categoria" value={filters.category} onChange={value => setFilters({ ...filters, category: value })} /><Field label="Preço mínimo" type="number" value={filters.min_price} onChange={value => setFilters({ ...filters, min_price: value })} /><Field label="Preço máximo" type="number" value={filters.max_price} onChange={value => setFilters({ ...filters, max_price: value })} /><div className="form-actions wide"><button className="primary">Aplicar filtros</button><button type="button" onClick={() => setFilters({ search: '', app_id: '', establishment_id: '', type: '', status: '', featured: '', category: '', min_price: '', max_price: '' })}>Limpar</button></div>
        </form>
      </Panel>
    </div>
...
function Audit({ payload, users, load }) {
  const [filters, setFilters] = useState({ search: '', user_id: '', action: '', entity_type: '', from: '', to: '' })
  const logs = payload?.logs || []
  useEffect(() => { if (!users) load('users') }, [])
  async function apply(event) { event.preventDefault(); const query = new URLSearchParams(Object.entries(filters).filter(([, value]) => value)); await load('audit', `/admin/ecosystem/audit?${query}`) }
  return <div className="admin-stack"><Panel title="Filtros da auditoria"><form className="filter-grid" onSubmit={apply}><Field label="Busca por ação, entidade, ID, IP ou usuário" value={filters.search} onChange={value => setFilters({ ...filters, search: value })} /><label>Administrador<select value={filters.user_id} onChange={event => setFilters({ ...filters, user_id: event.target.value })}><option value="">Todos</option>{users?.users?.map(user => <option key={user.id} value={user.id}>{fullName(user)} · {user.email}</option>)}</select></label><label>Ação<select value={filters.action} onChange={event => setFilters({ ...filters, action: event.target.value })}><option value="">Todas</option>{payload?.actions?.map(action => <option key={action}>{action}</option>)}</select></label><label>Entidade<select value={filters.entity_type} onChange={event => setFilters({ ...filters, entity_type: event.target.value })}><option value="">Todas</option>{payload?.entity_types?.map(type => <option key={type} value={type}>{type.split('\\').pop()}</option>)}</select></label><Field label="De" type="date" value={filters.from} onChange={value => setFilters({ ...filters, from: value })} /><Field label="Até" type="date" value={filters.to} onChange={value => setFilters({ ...filters, to: value })} /><div className="form-actions"><button className="primary">Aplicar filtros</button><button type="button" onClick={() => setFilters({ search: '', user_id: '', action: '', entity_type: '', from: '', to: '' })}>Limpar</button></div></form></Panel><Panel title={`Histórico administrativo (${logs.length})`}><div className="table-wrap"><table><thead><tr><th>Data</th><th>Administrador</th><th>Ação</th><th>Entidade</th><th>IP</th></tr></thead><tbody>{logs.map(log => <tr key={log.id}><td>{fmt(log.created_at)}</td><td>{log.user?.email || 'Sistema'}</td><td><b>{log.action}</b></td><td>{log.entity_type?.split('\\').pop() || '—'} {log.entity_id || ''}</td><td>{log.ip || '—'}</td></tr>)}</tbody></table></div></Panel></div>
}
