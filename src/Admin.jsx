...
const TABS = [
  ['command', 'Mission Control'], ['dashboard', 'Visão geral'], ['activity', 'Atividade'], ['financial', 'Financeiro'], ['applications', 'Aplicações'], ['users', 'Usuários'],
...
const tabFromLocation = () => window.location.pathname.replace(/\/+$/, '') === '/admin/financial' ? 'financial' : 'command'
...
export function AdminPage() {
  ...
  async function load(target = tab, customPath) {
    if (['financial', 'command'].includes(target)) {
      setProcessing(false)
      return null
    }
    setProcessing(true)
    setError('')
    try {
      const result = await request(customPath || paths[target])
  ...
  useEffect(() => {
    if (localStorage.getItem(TOKEN_KEY) && !['financial', 'command'].includes(tab) && !data[tab]) load(tab)
  }, [tab])
  ...
  async function mutate(path, options, success, reload = tab) {
    ...
    try {
      ...
      setMessage(success)
      await load(reload)
      if (reload !== 'dashboard') setData(prev => ({ ...prev, dashboard: undefined }))
      if (userDetail?.user?.id) setUserDetail(await request(`/admin/ecosystem/users/${userDetail.user.id}`))
      return true
    } catch (err) {
      setError(err.message)
      setProcessing(false)
  ...
  function changeTab(key) {
    ...
    setMessage('')
    setError('')
    const next = key === 'financial' ? '/admin/financial' : '/admin'
...
function Establishments({payload,applications,load,mutate}) {
  ...
  const [filters, setFilters] = useState({ search: '', app_id: '', approval: '' })
  ...
  async function ensureOwners() {
    ...
    try {
      const data = await request('/admin/ecosystem/users')
      setOwners(data?.users || [])
    } catch (error) {
      setLocalError(`Não foi possível carregar os responsáveis: ${error.message}`)
    }
  }
  ...
  function beginEdit(item) {
    ...
    setEditing(item.id)
    setOwnerSnapshot(item.user || null)
    setForm({
      name: item.name || '',
  ...
  function setPrimaryApplication(value) {
    ...
    setForm(current => {
      const appIds = appId && !current.app_ids.includes(appId) ? [...current.app_ids, appId] : current.app_ids
      return { ...current, app_id: appId, app_ids: appIds }
    })
  }
  ...
  function toggleApplication(appId, checked) {
    setForm(current => ({
      ...current,
      app_ids: checked
        ? [...new Set([...current.app_ids, Number(appId)])]
        : current.app_ids.filter(id => Number(id) !== Number(appId)),
    }))
  }
  ...
  async function save(event) {
    event.preventDefault()
    if (!form?.name?.trim()) {
      setLocalError('Informe o nome do estabelecimento.')
      return
    }
    ...
    const primaryAppId = Number(form.app_id) || appIds[0] || null
    if (primaryAppId && !appIds.includes(primaryAppId)) appIds.unshift(primaryAppId)

    const body = {
      name: form.name.trim(),
      fantasy: clean(form.fantasy),
      ...
      uf: clean(form.uf)?.toUpperCase() || null,
      cep: clean(form.cep),
    ...
    if (form.user_id) body.user_id = Number(form.user_id)
    if (primaryAppId) body.app_id = primaryAppId
    ...
    setLocalSuccess('')
    try {
      await request(`/admin/ecosystem/establishments/${editing}`, { method: 'PUT', body: JSON.stringify(body) })
      await load('establishments')
      setLocalSuccess('Dados do estabelecimento atualizados com sucesso.')
      setEditing(null)
      setForm(null)
      setOwnerSnapshot(null)
    } catch (error) {
  ...
  async function applyFilters(event) {
    ...
    await load('establishments', `/admin/ecosystem/establishments${query.toString() ? `?${query}` : ''}`)
  }

  function patch(item, changes) {
    mutate(`/admin/ecosystem/establishments/${item.id}`, { method: 'PUT', body: JSON.stringify(changes) }, 'Estabelecimento atualizado.', 'establishments')
  }

  const ownerOptions = [...owners]
  if (ownerSnapshot?.id && !ownerOptions.some(owner => Number(owner.id) === Number(ownerSnapshot.id))) ownerOptions.unshift(ownerSnapshot)

  return <div className="admin-stack">
    {localError && <div className="notice error">{localError}</div>}
    {localSuccess && <div className="notice success">{localSuccess}</div>}

    {editing && form && <Panel title={`Editar estabelecimento #${editing}`}>
      <form className="form-grid" onSubmit={save}>
        ...
        <Field label="CNPJ / documento" value={form.cnpj} onChange={value=>setForm({...form,cnpj:value})}/>
        ...
        <Field label="UF" value={form.uf} onChange={value=>setForm({...form,uf:value.slice(0,2).toUpperCase()})}/>
        ...
        <label>Responsável
          <select value={form.user_id} onChange={event=>setForm({...form,user_id:event.target.value})}>
            ...
            {ownerOptions.map(owner=><option key={owner.id} value={owner.id}>{fullName(owner)} · {owner.email}</option>)}
          </select>
        </label>

        <label>Aplicação principal
          <select value={form.app_id} onChange={event=>setPrimaryApplication(event.target.value)}>
            <option value="">Sem aplicação principal</option>
        ...
        <div className="wide">
          <p className="subheading">Aplicações vinculadas</p>
          <div className="access-grid">
            {apps.map(app => {
              const checked = form.app_ids.includes(Number(app.id))
              const primary = Number(form.app_id) === Number(app.id)
              return <label className={checked ? 'access active' : 'access'} key={app.id}>
                <input type="checkbox" checked={checked} disabled={primary} onChange={event=>toggleApplication(app.id,event.target.checked)}/>
                <b>{app.name}</b><small>{primary ? 'Aplicação principal' : checked ? 'Vinculada' : 'Não vinculada'}</small>
              </label>
            })}
          </div>
        </div>
        ...
        <div className="wide">
          <p className="subheading">Estado administrativo</p>
          <div className="access-grid">
            <label className={form.is_approved?'access active':'access'}><input type="checkbox" checked={form.is_approved} onChange={event=>setForm({...form,is_approved:event.target.checked})}/><b>Aprovado</b><small>libera o estabelecimento</small></label>
            <label className={form.is_published?'access active':'access'}><input type="checkbox" checked={form.is_published} onChange={event=>setForm({...form,is_published:event.target.checked})}/><b>Publicado</b><small>visível publicamente</small></label>
            <label className={form.is_featured?'access active':'access'}><input type="checkbox" checked={form.is_featured} onChange={event=>setForm({...form,is_featured:event.target.checked})}/><b>Destaque</b><small>prioridade de exibição</small></label>
            <label className={form.is_cancelled?'access active':'access'}><input type="checkbox" checked={form.is_cancelled} onChange={event=>setForm({...form,is_cancelled:event.target.checked})}/><b>Cancelado</b><small>registro desativado</small></label>
          </div>
        </div>

        <div className="form-actions wide">
          <button className="primary" disabled={saving}>{saving?'Salvando...':'Salvar alterações'}</button>
          <button type="button" disabled={saving} onClick={closeEditor}>Cancelar</button>
        </div>
      </form>
    </Panel>}

    <Panel title="Localizar estabelecimentos">
      <form className="filter-grid" onSubmit={applyFilters}>
        <Field label="Busca" value={filters.search} onChange={value=>setFilters({...filters,search:value})}/>
        ...
        <label>Aprovação<select value={filters.approval} onChange={event=>setFilters({...filters,approval:event.target.value})}><option value="">Todos</option><option value="approved">Aprovados</option><option value="pending">Pendentes</option></select></label>
        <button className="primary">Buscar</button>
      </form>
      <p className="helper">A busca permite localizar e editar qualquer estabelecimento do ecossistema, inclusive registros que não estejam entre os mais recentes.</p>
    </Panel>

    <Panel title={`Estabelecimentos (${rows.length})`}>
      <div className="table-wrap"><table><thead><tr><th>Estabelecimento</th><th>Aplicação</th><th>Responsável</th><th>Localidade</th><th>Status</th><th>Ações</th></tr></thead><tbody>{rows.length?rows.map(item=><tr key={item.id}><td><b>{item.fantasy||item.name}</b><small>#{item.id} · {item.cnpj||'sem documento'}</small></td><td>{item.app?.name||item.application?.name||item.applications?.map(app=>app.name).join(', ')||'—'}</td><td>{item.user?.email||'—'}</td><td>{[item.city,item.uf].filter(Boolean).join(' / ')||'—'}</td><td><span className={item.is_approved?'status on':'status'}>{item.is_approved?'Aprovado':'Pendente'}</span></td><td className="row-actions"><button className="primary" onClick={()=>beginEdit(item)}>Editar dados</button><button onClick={()=>patch(item,{is_approved:!item.is_approved})}>{item.is_approved?'Retirar aprovação':'Aprovar'}</button></td></tr>):<tr><td colSpan="6">Nenhum estabelecimento encontrado.</td></tr>}</tbody></table></div>
    </Panel>
  </div>
}

function Items({payload}){const rows=payload?.items||payload?.data||[];return <Panel title={`Itens (${rows.length})`}><div className="table-wrap"><table><thead><tr><th>ID</th><th>Item</th><th>Aplicação</th><th>Estabelecimento</th><th>Preço</th><th>Status</th></tr></thead><tbody>{rows.length?rows.map(item=><tr key={item.id}><td>#{item.id}</td><td><b>{item.name}</b><small>{item.slug||''}</small></td><td>{item.application?.name||item.app?.name||item.app_id||'—'}</td><td>{item.establishment?.fantasy||item.establishment?.name||item.establishment_id||'—'}</td><td>{item.price!=null?Number(item.price).toLocaleString('pt-BR',{style:'currency',currency:'BRL'}):'—'}</td><td><span className={item.is_active===false?'status':'status on'}>{item.is_active===false?'Inativo':'Ativo'}</span></td></tr>):<tr><td colSpan="6">Nenhum item encontrado.</td></tr>}</tbody></table></div></Panel>}

function SiteSettings({payload,mutate}){const[site,setSite]=useState(payload?.site||{});useEffect(()=>{if(payload?.site)setSite(payload.site)},[payload]);const contact=site.contact||{};function save(e){e.preventDefault();mutate('/admin/ecosystem/settings',{method:'PUT',body:JSON.stringify({site})},'Site institucional atualizado.','site')}return <form onSubmit={save} className="admin-stack"><Panel title="Contato"><div className="form-grid"><Field label="E-mail" value={contact.email||''} onChange={v=>setSite({...site,contact:{...contact,email:v}})}/><Field label="Instagram" value={contact.instagram||''} onChange={v=>setSite({...site,contact:{...contact,instagram:v}})}/></div></Panel><button className="primary save-site">Publicar alterações do site</button></form>}
...
function Timeline({rows,compact=false}){if(!rows?.length)return <Empty/>;return <div className={`timeline ${compact?'compact':''}`}>{rows.map(row=><article className="timeline-row" key={row.id}><span className="timeline-dot"/><div><div className="timeline-title"><b>{humanType(row.type)}</b><i>{row.application?.name||'Aplicação não identificada'}</i></div><p>{row.user?`${fullName(row.user)} · ${row.user.email}`:'Visitante / sistema'}{row.name?` · ${row.name}`:''}</p><small>{fmt(row.created_at)}</small></div></article>)}</div>}
function humanType(type=''){const map={login:'Login',login_google:'Login Google',logout:'Logout',register:'Cadastro',view:'Visualização',update:'Atualização'};return map[type]||type.replaceAll('_',' ').replace(/\b\w/g,x=>x.toUpperCase())}
