...
function buildAnalytics(payload) {
  const hourly = Array.isArray(payload?.interaction_series)
  ...
  const applications = Array.isArray(payload?.applications)
    ...
    : []
  const usersByApp = Array.isArray(payload?.applications)
    ...
    : []
  const activityTypes = Array.isArray(payload?.activity_types)
  ...
  return `
    <section class="ecosystem-insights" aria-label="Análises visuais do ecossistema">
      ...
      <div class="ecosystem-insights__grid">
        <peter-insight-chart type="line" title="Interações nas últimas 24 horas" subtitle="Volume total por hora comparado aos erros registrados." data="${dataAttribute(hourly)}" label-key="label" value-key="total" secondary-key="errors" primary-label="Interações" secondary-label="Erros"></peter-insight-chart>
        <peter-insight-chart type="bar" title="Uso das aplicações" subtitle="Interações registradas por aplicação nos últimos 30 dias." data="${dataAttribute(applications)}" label-key="label" value-key="interactions" primary-label="Interações 30d" format="compact"></peter-insight-chart>
        <peter-insight-chart type="bar" title="Alcance e recorrência" subtitle="Usuários vinculados comparados aos usuários ativos nos últimos 30 dias." data="${dataAttribute(usersByApp)}" label-key="label" value-key="users" secondary-key="active" primary-label="Usuários" secondary-label="Ativos 30d"></peter-insight-chart>
        <peter-insight-chart type="donut" title="Tipos de interação" subtitle="Distribuição das ações mais frequentes registradas pela telemetria central." data="${dataAttribute(activityTypes)}" primary-label="Interações" format="compact"></peter-insight-chart>
      </div>
    </section>
  `
}
...
function renderAnalytics() {
  ...
  host.innerHTML = buildAnalytics(latestPayload)
}
