...
function searchRequest(query, signal) {
  ...
  return fetch(`${API}/admin/ecosystem/command/search?q=${encodeURIComponent(query)}`, {
    ...
    if (!response.ok) {
      throw new Error(payload?.error || payload?.message || 'Não foi possível realizar a pesquisa.')
    }
    return payload
  })
}
...
function AdminSearchPage({ onClose }) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      setLoading(true)
      try {
        setResult(await searchRequest(value, controller.signal))
      } catch (requestError) {
  ...
  return <section className="admin-global-search-page" aria-label="Pesquisa global do Admin Center">
    <div className="global-search-page-head">
      ...
      <div>
        ...
        <h1>Pesquisar no ecossistema</h1>
        <p>Encontre usuários, aplicações, estabelecimentos, itens, eventos, pedidos e pagamentos em um único lugar.</p>
      </div>
    </div>
    ...
    <div className="global-search-input-wrap">
      ...
      <input
        ...
        placeholder="Digite nome, e-mail, ID, slug, item, estabelecimento, pedido..."
        aria-label="Pesquisar em todo o ecossistema"
        autoComplete="off"
      />
      {query && <button type="button" onClick={() => setQuery('')} aria-label="Limpar pesquisa">×</button>}
    </div>
    ...
    <div className="global-search-body">
      ...
      {!loading && !error && result && total > 0 && <div className="global-search-results">
        ...
        {visibleGroups.map(([key, label]) => {
          ...
          return <section className="global-search-group" key={key}>
            <header><div><span>{label}</span><b>{rows.length}</b></div></header>
            <div className="global-search-grid">
              {rows.map((row, index) => {
                const meta = resultMeta(row)
                const href = row?.url || row?.reference_url || null
                return <article className="global-search-card" key={`${key}-${row?.id ?? row?.public_id ?? index}`}>
                  <div className="global-search-card-main"><span className="global-search-kind">{label.slice(0, -1) || label}</span><h3>{resultTitle(row)}</h3><p>{resultSubtitle(row)}</p>{meta.length > 0 && <div className="global-search-meta">{meta.map(value => <span key={value}>{value}</span>)}</div>}</div>
                  <div className="global-search-card-side"><small>{row?.id != null ? `#${row.id}` : row?.public_id || ''}</small>{href && <a href={href} target="_blank" rel="noreferrer">Abrir ↗</a>}</div>
                </article>
              })}
            </div>
          </section>
...
export default function AdminGlobalSearch() {
  ...
  useEffect(() => {
    const syncTargets = () => {
      setTopActions(document.querySelector('.top-actions'))
  ...
  useEffect(() => {
    if (!topActions || !workspace) setOpen(false)
  }, [topActions, workspace])
  ...
  useEffect(() => {
    const handleNavigation = event => {
      if (event.target.closest('.sidebar nav button, .sidebar > .brand')) setOpen(false)
    }
  ...
  useEffect(() => {
    document.body.classList.toggle('admin-global-search-open', open)
    return () => document.body.classList.remove('admin-global-search-open')
  }, [open])
