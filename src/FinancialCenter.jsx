...
export default function FinancialCenter({ embedded = false }) {
  ...
  const content = <div className="finance-stack">
    <section className="finance-hero">
      <div><p>MISSION FINANCIAL CONTROL</p><h2>Centro financeiro do ecossistema</h2><span>Caixa só reconhece pagamento confirmado. Carrinho, checkout ou QR Code gerado ficam em aberto até o dinheiro ser efetivamente aprovado.</span></div>
    ...
    {tab === 'overview' && <>
      <div className="finance-kpis">
        ...
        <Kpi title="Receita Peter Tecnet realizada" value={money(s.totals?.platform_fees)} sub="somente pagamentos confirmados" />
        ...
        <Kpi title="Líquido produtores" value={money(s.totals?.seller_net)} sub="somente vendas pagas" />
        <Kpi title="Taxas de gateway realizadas" value={money(s.totals?.provider_fees)} sub="somente pagamentos confirmados" />
        <Kpi title="Aprovação" value={pct(approved,total)} sub={`${approved} de ${total} tentativas`} danger={total>0 && approved/total<.8} />
      </div>

      <Panel title="Conciliação de caixa" subtitle={reconciliation.recognition_rule || 'Pagamento pendente ou QR gerado nunca é contabilizado como dinheiro recebido.'}>
        <Table heads={['Estado','Transações','Valor bruto','Receita Peter','Entra no caixa?']} rows={[
          ['Confirmado', reconciliation.realized?.count || 0, money(reconciliation.realized?.amount), money(reconciliation.realized?.platform_fees), 'Sim'],
          ['Em aberto / QR gerado', reconciliation.open?.count || 0, money(reconciliation.open?.amount), money(reconciliation.open?.platform_fees), 'Não'],
          ['Falhou / cancelou', reconciliation.failed?.count || 0, money(reconciliation.failed?.amount), money(reconciliation.failed?.platform_fees), 'Não'],
          ['Estornado / chargeback', reconciliation.reversed?.count || 0, money(reconciliation.reversed?.amount), money(reconciliation.reversed?.platform_fees), 'Não'],
        ]}/>
      </Panel>
      ...
      <section className="finance-panel"><header><div><b>Filtros de investigação</b><small>O período financeiro usa a data da confirmação, estorno ou falha — não apenas a data em que o QR foi criado.</small></div><button onClick={()=>setF({app_slug:'',provider:'',status:'',method:'',from:'',to:''})}>Limpar</button></header><div className="finance-filters"><input placeholder="Aplicação (slug)" value={f.app_slug} onChange={e=>setF({...f,app_slug:e.target.value})}/><input placeholder="Gateway / provedor" value={f.provider} onChange={e=>setF({...f,provider:e.target.value})}/><select value={f.method} onChange={e=>setF({...f,method:e.target.value})}><option value="">Todos os métodos</option><option value="pix">PIX</option><option value="card">Cartão</option></select><select value={f.status} onChange={e=>setF({...f,status:e.target.value})}><option value="">Todos os status</option><option value="paid">Pago</option><option value="pending">Pendente</option><option value="in_process">Processando</option><option value="failed">Falhou</option><option value="rejected">Rejeitado</option><option value="cancelled">Cancelado</option><option value="refunded">Estornado</option><option value="charged_back">Chargeback</option></select><input type="date" value={f.from} onChange={e=>setF({...f,from:e.target.value})}/><input type="date" value={f.to} onChange={e=>setF({...f,to:e.target.value})}/></div></section>
      ...
      <div className="finance-grid"><Panel title="Métodos de pagamento" subtitle="PIX, cartão e demais canais"><Table heads={['Método','Tx','Aprov.','Pend.','Falhas','Volume pago','Em aberto']} rows={d?.methods?.map(x=>[label(x.method),x.transactions,x.approved,x.pending||0,x.failed,money(x.gross),money(x.pending_gross)])}/></Panel><Panel title="Economia Cutinapp" subtitle="Venda aprovada e obrigações"><Table heads={['Indicador','Valor']} rows={[[ 'GMV pago', money(commerce.paid_gross) ],['Receita plataforma',money(commerce.platform_revenue)],['Líquido produtor',money(commerce.producer_net)],['Pedidos cancelados',commerce.cancelled_orders||0],['Estornos',commerce.refunded_orders||0]]}/></Panel></div>
