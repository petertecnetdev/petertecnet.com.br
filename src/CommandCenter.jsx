...
export default function CommandCenter() {
  ...
  return <div className="cc-stack">
    ...
    <div className="cc-toolbar">
      <nav>{[['overview','Operações'],['search','Busca global'],['incidents','Incidentes'],['security','Segurança'],['queues','Filas & runtime']].map(([k,l]) => <button key={k} className={section===k?'active':''} onClick={()=>navigateSection(k)}>{l}</button>)}</nav>
    ...
    {section === 'overview' && <>
      <div className="cc-kpis">
        ...
        <Kpi title="Eventos críticos 24h" value={overview?.security?.critical_events_24h || 0} sub={`${overview?.security?.errors_24h || 0} erros · veja Segurança`} danger={overview?.security?.critical_events_24h > 0} onClick={()=>openKpi('security')} destination="Investigar eventos críticos"/>
      </div>
    ...
    {section === 'security' && <div id="cc-detail-security" className={focusTarget==='security'?'cc-focus':''}><CriticalEventsPanel security={security}/></div>}
