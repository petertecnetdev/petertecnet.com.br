...
export default function CommandCenter() {
  ...
  async function load(silent = false) {
    ...
    try {
      ...
      const operationalIssues = await api('/admin/ecosystem/command/issues?status=all&per_page=100')
      setOverview(o); setSecurity(s); setQueues(q); setIncidents(i); setIssues(operationalIssues)
  ...
  return <div className="cc-stack">
    <section className="cc-hero">
      <div><p>MISSION CONTROL · PETER TECNET</p><h2>Centro operacional do ecossistema</h2><span>Disponibilidade, problemas, segurança, filas, incidentes, backup, aplicações e busca global em uma única sala de controle.</span></div>
    ...
    {section === 'issues' && <OperationalIssuesPanel issues={issues} onRefresh={()=>load()} onUpdate={updateOperationalIssue} onCreateIncident={createIssueIncident} fetchDetails={id=>api(`/admin/ecosystem/command/issues/${id}`)}/>} 
